...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Lấy thông tin đơn hàng từ biểu mẫu
    $sql = "INSERT INTO order_history (user_id, fullname, email, phone_number, address, payment_method, note, total_amount, order_date) 
            VALUES ('$user_id', '$fullname', '$email', '$phone_number', '$address', '$payment_method', '$note', '$total', NOW())";
    execute($sql);

    // Lấy order_id vừa được thêm vào
    $order_id = mysqli_insert_id($con); // Thay bằng mysqli_insert_id($conn)
    ...
    foreach ($cart as $item) {
        ...
        $sql = "INSERT INTO order_details (order_id, product_id, num, price, id_user) 
                VALUES ('$order_id', '$product_id', '$num', '$price', '$user_id')";
        execute($sql);
    }

    // Xóa giỏ hàng sau khi đặt hàng thành công
    setcookie('cart', '', time() - 3600, '/'); // Xóa cookie giỏ hàng
    unset($_SESSION['cart']); // Xóa giỏ hàng khỏi phiên làm việc hiện tại
}
?>
<!DOCTYPE html>
<html lang="en">
                    ...
                    <div class="container">
                        ...
                        <div class="row">
                            <div class="panel panel-primary col-md-6">
                                ...
                                <div class="form-group">
                                    <label style="text-align: left;padding: 0.5rem 0; border-bottom:0.5px solid black;" for="payment_method">Phương thức thanh toán:</label>
                                    <select class="form-control" id="payment_method" name="payment_method">
                                        <option value="visa">Visa</option>
                                        <option value="zalo_pay">Thanh toán qua Zalo Pay</option>
                                        <option value="momo">Thanh toán qua Momo</option>
                                        <option value="tghino">Thẻ tín dụng/ghi nợ </option>
